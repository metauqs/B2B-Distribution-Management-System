  ...
  const [
    ...
    prisma.$queryRaw<Array<{
      ...
      SELECT
        ...
        (SELECT COALESCE(SUM(amount), 0)::float FROM collections WHERE "deletedAt" IS NULL AND status != 'CANCELLED' AND (${rawBranchId} = '' OR "branchId" = ${rawBranchId}) AND (${fromDate}::timestamptz IS NULL OR date >= ${fromDate}) AND (${toDate}::timestamptz IS NULL OR date <= ${toDate})) as "totalCollections",
        (SELECT COUNT(id)::int FROM collections WHERE "deletedAt" IS NULL AND status != 'CANCELLED' AND (${rawBranchId} = '' OR "branchId" = ${rawBranchId}) AND (${fromDate}::timestamptz IS NULL OR date >= ${fromDate}) AND (${toDate}::timestamptz IS NULL OR date <= ${toDate})) as "collectionCount",
